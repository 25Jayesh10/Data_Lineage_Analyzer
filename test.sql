...
END
CREATE PROCEDURE sp_process_and_log
    @client_id INT,
    @from_date DATE,
    @to_date DATE
AS
BEGIN
    -- Execute the procedure to sum client orders
    EXEC sp_sum_client_orders @client_id, @from_date, @to_date;

    -- Execute the procedure to log HR employees
    EXEC log_hr_employees;
END
GO